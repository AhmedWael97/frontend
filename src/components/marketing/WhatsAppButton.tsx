...
/**
 * Persistent WhatsApp quick-chat button, shown to everyone (guest or signed
 * in) — a mostly-Egypt/Saudi, WhatsApp-first audience wants this even mid
 * upgrade decisions, alongside SupportChatBubble (opposite corner, so the
 * two never overlap). Sits above MobileCtaBar's ~88px sticky strip on
 * mobile (bottom-24) so it doesn't collide with it; MobileCtaBar is
 * md:hidden, so bottom-5 is safe again from md up.
 */
export default function WhatsAppButton() {
  ...
  return (
    <a
      ...
      className="fixed bottom-24 md:bottom-5 start-5 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-xl transition hover:scale-105"
    >
