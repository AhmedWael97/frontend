...
// ── Deterministic sample data (no random → no hydration drift) ────────────────
const CHART = [
...
// Sample heatmap click dots (x%, y%, intensity)
const DOTS = [
    ...
    <Card>
      <CardContent className="p-5">
        <div className="flex items-start justify-between">
          ...
          <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center ltr:ml-3 rtl:mr-3">
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-primary/30 bg-primary/5 p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-on-surface">This is a live demo with sample data</p>
            <p className="text-xs text-on-surface-variant mt-0.5">Add your website to see your <strong>own</strong> visitors, heatmaps and insights — it takes ~2 minutes.</p>
          </div>
        </div>
        <Link href={`/${locale}/settings/domains?welcome=1`}>
          <Button className="gap-2 shrink-0"><Globe className="w-4 h-4" /> Add my website <ArrowRight className="w-4 h-4" /></Button>
        </Link>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        <Kpi label="Visitors" value="2,847" icon={Users} />
        <Kpi label="Sessions" value="3,912" icon={Activity} />
        <Kpi label="Avg. time" value="2m 14s" icon={Clock} />
        <Kpi label="Bounce rate" value="38.2%" icon={TrendingDown} hint="Healthy — below 50%" />
        <Kpi label="Hot leads" value="64" icon={Flame} hint="Most likely to convert" />
      </div>

      <Card>
        <CardHeader><CardTitle className="text-sm font-semibold uppercase tracking-widest text-on-surface-variant">Visitors & sessions (last 14 days)</CardTitle></CardHeader>
        <CardContent>
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={CHART}>
              <defs>
                <linearGradient id="d-v" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#c0c1ff" stopOpacity={0.3} /><stop offset="95%" stopColor="#c0c1ff" stopOpacity={0} /></linearGradient>
                <linearGradient id="d-s" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#d0bcff" stopOpacity={0.2} /><stop offset="95%" stopColor="#d0bcff" stopOpacity={0} /></linearGradient>
              </defs>
              ...
              <Area type="monotone" dataKey="visitors" stroke="#c0c1ff" strokeWidth={2} fill="url(#d-v)" />
              <Area type="monotone" dataKey="sessions" stroke="#d0bcff" strokeWidth={2} fill="url(#d-s)" />
            </AreaChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card className="lg:col-span-2">
          <CardHeader><CardTitle className="text-sm font-semibold uppercase tracking-widest text-on-surface-variant flex items-center gap-2"><MousePointerClick className="w-4 h-4 text-primary" /> Top pages</CardTitle></CardHeader>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Heatmap preview */}
        <Card>
          <CardHeader><CardTitle className="text-sm font-semibold uppercase tracking-widest text-on-surface-variant">Click heatmap</CardTitle></CardHeader>
          <CardContent>
            <div className="relative w-full aspect-[16/10] rounded-lg bg-surface-container-lowest border border-outline-variant/20 overflow-hidden">
              ...
              {DOTS.map(([x, y, i], idx) => (
                <span key={idx} className="absolute rounded-full"
                  style={{ left: `${x}%`, top: `${y}%`, width: 26, height: 26, transform: "translate(-50%,-50%)",
                    background: `radial-gradient(circle, rgba(239,68,68,${i}) 0%, rgba(239,68,68,0) 70%)` }} />
              ))}
            </div>
            <p className="text-xs text-on-surface-variant mt-2">Where visitors actually click — the brightest spots get the most attention.</p>
          </CardContent>
        </Card>

        {/* Countries */}
        <Card>
          <CardHeader><CardTitle className="text-sm font-semibold uppercase tracking-widest text-on-surface-variant flex items-center gap-2"><Globe className="w-4 h-4 text-sky-400" /> Top countries</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            {COUNTRIES.map((c) => (
      ...
      <div className="rounded-xl border border-primary/30 bg-primary/5 p-5 text-center space-y-3">
        <p className="text-base font-bold text-on-surface">Ready to see your own data?</p>
        <p className="text-sm text-on-surface-variant">Add your website and start tracking real visitors in minutes.</p>
        <Link href={`/${locale}/settings/domains?welcome=1`}>
          <Button className="gap-2"><Globe className="w-4 h-4" /> Add my website</Button>
        </Link>
      </div>
    </div>
  );
