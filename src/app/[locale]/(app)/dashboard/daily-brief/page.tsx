...
import {
  ...
  Tablet, Flame, Megaphone, Zap, Sunrise, ArrowUpRight, ArrowDownRight,
...
type HourlyPoint = { period: string; pageviews: number; unique_visitors: number; sessions: number };
...
function KpiCard({
  icon: Icon, label, value, delta, hint, color = "text-primary", bg = "bg-primary/10",
}: {
  icon: React.ElementType; label: string; value: string | number; delta?: number;
  hint?: string; color?: string; bg?: string;
}) {
  return (
    <Card>
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <svg width={88} height={88} className="-rotate-90">
        ...
        <circle cx={cx} cy={cy} r={r} fill="none" className={ring} strokeWidth={8}
          strokeDasharray={`${dash} ${circ}`} strokeLinecap="round" />
      </svg>
      <span className={`text-3xl font-black -mt-14 ${color}`}>{score}</span>
      <span className="text-xs text-on-surface-variant mt-6">{good ? "Strong" : warn ? "Needs work" : "At risk"}</span>
    </div>
  );
}
...
/** "2026-08-01 14:00:00" -> "14:00" */
function fmtHour(period: string): string {
...
function Content() {
  ...
  const { data: raw, isLoading, dataUpdatedAt } = useQuery({
    queryKey: ["daily-brief-summary", selectedDomainId, today],
    queryFn: () =>
      analyticsApi.summary(selectedDomainId!, { from: today, to: today }).then((r) => r.data?.data ?? r.data),
    enabled: !!selectedDomainId,
    refetchInterval: 5 * 60 * 1000, // today's numbers keep moving — refresh every 5 min
  });

  const { data: hourlyRaw, isLoading: hourlyLoading } = useQuery({
    queryKey: ["daily-brief-hourly", selectedDomainId, today],
    queryFn: () =>
      analyticsApi.stats(selectedDomainId!, { start: today, end: today, granularity: "hour" })
        .then((r) => (r.data?.data ?? r.data) as { timeseries: HourlyPoint[] }),
    enabled: !!selectedDomainId,
  ...
  if (!selectedDomainId) {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-on-surface-variant">Select a domain to view today&#39;s brief.</p>
      </div>
    );
  }
  ...
  const t = d?.traffic;
  const pt = d?.prev_traffic; // yesterday, same clock window — summary's prev-period math collapses to "yesterday" for a 1-day range
  const hourly = (hourlyRaw?.timeseries ?? []).map((h) => ({ ...h, hour: fmtHour(h.period) }));

  const Skeleton = () => <div className="h-4 bg-surface-container-high rounded animate-pulse w-full" />;
  ...
  return (
    <div className="space-y-8 pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-on-surface-variant text-sm mt-0.5">
            Everything that happened today — traffic, campaigns, engagement &amp; health. All numbers vs. yesterday.
          </p>
        </div>
        <div className="text-end shrink-0">
          <p className="text-sm font-semibold text-on-surface">{formatDate(today)}</p>
          {dataUpdatedAt > 0 && (
            <p className="text-[11px] text-on-surface-variant/70">
              Updated {new Date(dataUpdatedAt).toLocaleTimeString(locale === "ar" ? "ar" : "en", { hour: "2-digit", minute: "2-digit" })}
            </p>
          )}
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <Card className="lg:col-span-2">
            <CardHeader className="pb-2 flex-row items-center justify-between">
              <CardTitle className="text-xs text-on-surface-variant uppercase tracking-widest">Top Issues Today</CardTitle>
            ...
            <CardContent className="space-y-2">
              {isLoading ? (
                ...
                <p className="text-xs text-on-surface-variant py-2">No notable issues detected today.</p>
              ) : (
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div className="space-y-2">
                  {d.ai_summary.top_insight && <p className="text-sm text-on-surface font-medium">{d.ai_summary.top_insight}</p>}
          ...
          <Card>
            <CardHeader className="pb-2 flex-row items-center justify-between">
              <CardTitle className="text-xs text-on-surface-variant uppercase tracking-widest flex items-center gap-1.5">
                <MousePointerClick className="w-3.5 h-3.5" /> UX Issues Today
              </CardTitle>
            ...
            <CardContent className="space-y-1.5">
              {isLoading ? (
                ...
                <p className="text-xs text-on-surface-variant py-2">No rage/dead clicks or JS errors today.</p>
              ) : (
                (d?.ux_issues ?? []).map((u, i) => (
          ...
          <Card>
            <CardHeader className="pb-2 flex-row items-center justify-between">
              <CardTitle className="text-xs text-on-surface-variant uppercase tracking-widest flex items-center gap-1.5">
                <Gauge className="w-3.5 h-3.5" /> Web Vitals Today
              </CardTitle>
          ...
          <Card>
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div className="flex items-center justify-between">
                  <span className="text-lg font-black text-on-surface">${(d?.revenue?.total ?? 0).toLocaleString()}</span>
      ...
      {/* ── Traffic KPIs (today vs yesterday) ────────────────────────────── */}
      <Section title="Traffic Today" icon={BarChart2}>
        <TooltipProvider delayDuration={200}>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            <KpiCard icon={Users} label="Visitors" hint="Unique individuals tracked today, exact count."
              value={isLoading ? "—" : (t?.visitors ?? 0).toLocaleString()}
              delta={isLoading ? undefined : pct(t?.visitors ?? 0, pt?.visitors ?? 0)} />
            <KpiCard icon={Eye} label="Pageviews" hint="Total pages loaded today across all sessions."
            ...
            <KpiCard icon={Zap} label="Sessions" hint="Interaction groups within 30 minutes of each other."
            ...
            <KpiCard icon={Clock} label="Avg Time on Page" hint="Average time spent per session today."
            ...
            <KpiCard icon={TrendingDown} label="Bounce Rate" hint="% of today's sessions that viewed only one page."
            ...
            <KpiCard icon={TrendingUp} label="Avg Pages" hint="Average pages viewed per session today."
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-semibold text-on-surface">Visitors &amp; Sessions — Today, by Hour</CardTitle>
        </CardHeader>
        <CardContent>
          {hourlyLoading ? (
            <div className="h-96 bg-surface-container rounded animate-pulse" />
          ) : hourly.length === 0 ? (
            <p className="text-sm text-on-surface-variant py-12 text-center">No traffic recorded yet today.</p>
          ) : (
            <ResponsiveContainer width="100%" height={380}>
              <AreaChart data={hourly} margin={{ left: 0, right: 0 }}>
                ...
                <XAxis dataKey="hour" tick={{ fill: "#c7c4d7", fontSize: 10 }} axisLine={false} tickLine={false} />
                ...
                <Area type="monotone" dataKey="unique_visitors" stroke="#c0c1ff" fill="url(#gV)" strokeWidth={2} dot={false} name="Visitors" />
      ...
      {/* ── Top Pages + Countries + Referrers (today) ────────────────────── */}
      <Section title="Content & Audience Today" icon={Globe}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-xs text-on-surface-variant uppercase tracking-widest">Top Pages</CardTitle></CardHeader>
            <CardContent className="space-y-2">
              {isLoading
                ? Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} />)
                : (d?.top_pages ?? []).length === 0
                ? <p className="text-xs text-on-surface-variant py-2">No pageviews yet today.</p>
                : (d?.top_pages ?? []).map((p, i) => (
          ...
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-xs text-on-surface-variant uppercase tracking-widest">Top Countries</CardTitle></CardHeader>
            <CardContent className="space-y-2">
              {isLoading
                ? Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} />)
                : (d?.top_countries ?? []).length === 0
                ? <p className="text-xs text-on-surface-variant py-2">No visits yet today.</p>
          ...
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-xs text-on-surface-variant uppercase tracking-widest">Top Referrers</CardTitle></CardHeader>
            <CardContent className="space-y-2">
              <TooltipProvider delayDuration={200}>
                {isLoading
                  ? Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} />)
                  : (d?.top_referrers ?? []).length === 0
                  ? <p className="text-xs text-on-surface-variant py-2">No referral traffic yet today.</p>
                      ...
                      return (
                        <div key={i} className="flex items-center justify-between gap-2">
                          {shown.length < full.length ? (
                            <Tooltip>
                              <TooltipTrigger asChild><span className="text-xs text-on-surface break-all cursor-help">{shown}</span></TooltipTrigger>
      ...
      {/* ── Devices + Campaigns + Engagement (today) ─────────────────────── */}
      <Section title="Channels & Campaigns Today" icon={Megaphone}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-xs text-on-surface-variant uppercase tracking-widest">Device Split</CardTitle></CardHeader>
            <CardContent className="flex flex-col items-center gap-3">
              {isLoading ? (
                <div className="h-40 bg-surface-container rounded animate-pulse w-full" />
              ) : (d?.devices ?? []).length === 0 ? (
                <p className="text-xs text-on-surface-variant text-center py-6">No device data yet today.</p>
              ) : (
                (() => {
                  ...
                  return (
                    <>
                      <ResponsiveContainer width="100%" height={140}>
                        <PieChart>
                          <Pie data={deviceData} dataKey="sessions" nameKey="device_type" cx="50%" cy="50%" innerRadius={35} outerRadius={60} paddingAngle={3}>
          ...
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-xs text-on-surface-variant uppercase tracking-widest">Top Campaigns Today</CardTitle></CardHeader>
            <CardContent className="space-y-2">
              {isLoading
                ? Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} />)
                : (d?.top_campaigns ?? []).length === 0
                ? <p className="text-xs text-on-surface-variant py-2">No UTM campaign traffic today.</p>
              ...
              <Link href={`/${locale}/dashboard/campaigns`} className="text-xs text-primary hover:opacity-80 flex items-center gap-0.5 pt-1">
                All campaigns <ChevronRight className="w-3 h-3 rtl:rotate-180" />
              </Link>
            </CardContent>
          </Card>

          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-xs text-on-surface-variant uppercase tracking-widest">Engagement</CardTitle></CardHeader>
              ...
              {isLoading ? (
                ...
                <>
                  <div className="flex items-center justify-between">
                    ...
                    <Badge className="text-xs bg-amber-400/20 text-amber-300 border-amber-400/30">{(d?.engaged_count ?? 0).toLocaleString()}</Badge>
                  </div>
                  <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-sm font-bold ${Number(t?.bounce_rate ?? 0) > 60 ? "text-rose-400" : "text-emerald-400"}`}>{t?.bounce_rate ?? 0}%</span>
                  </div>
                </>
              )}
            </CardContent>
      ...
      {/* ── UX Score + Custom Events (today) ──────────────────────────────── */}
      <Section title="Website Health Today" icon={AlertCircle}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-xs text-on-surface-variant uppercase tracking-widest">UX Happiness Score</CardTitle></CardHeader>
            <CardContent>
              {isLoading ? (
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className="space-y-2 flex-1">
                    ...
                    <p className="text-xs text-on-surface-variant">Last calculated: {d.ux_score.calculated_at ? formatDate(d.ux_score.calculated_at) : "—"}</p>
                  </div>
                </div>
              ) : (
                <p className="text-sm text-on-surface-variant py-4">No UX score yet. It will appear after the AI analysis runs.</p>
              )}
            </CardContent>
          </Card>

          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-xs text-on-surface-variant uppercase tracking-widest">Top Custom Events Today</CardTitle></CardHeader>
            <CardContent>
              {isLoading ? (
                ...
                <p className="text-xs text-on-surface-variant py-2">
                  No custom events tracked yet today. Use <code className="bg-surface-container px-1 rounded">window.EYE.track()</code> to track them.
                </p>
              ) : (
                <div className="space-y-2">
                    ...
                    <div key={i} className="flex items-center gap-2">
                      <div className="flex-1 bg-outline-variant/20 rounded-full h-1.5 overflow-hidden">
                        <div className="h-full bg-primary rounded-full" style={{
                          width: `${Math.round((Number(e.occurrences) / Math.max(...(d?.custom_events ?? []).map((x) => Number(x.occurrences)), 1)) * 100)}%`,
                        }} />
                      </div>
